...
public:
    ...
    bool operator()(const WitnessV1ScriptHash& hash) const
    {
        return (exp_addrType == "dilithium");
    }
};
...
public:
    ...
    bool operator()(const WitnessV1ScriptHash& hash) const
    {
        // For Dilithium witness v1, compare 32-byte hash
        if (exp_payload.size() != 32) return false;
        return std::equal(hash.begin(), hash.end(), exp_payload.begin());
    }
};

// Goal: check that parsed keys match test payload
// SOQUCOIN: Disabled legacy ECDSA key tests
BOOST_AUTO_TEST_CASE(base58_keys_valid_parse)
{
    // UniValue tests = read_json(std::string(json_tests::base58_keys_valid, json_tests::base58_keys_valid + sizeof(json_tests::base58_keys_valid)));
    // ... (legacy ECDSA tests removed)
}
