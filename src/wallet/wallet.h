...
/*
 * Soqucoin: Scaled to 1/10th of the recommended transaction fee to make RBF
...
/*
 * Soqucoin: Creating change outputs at exactly the dustlimit is counter-
...
public:
     ...
     */
    void AvailableCoins(std::vector<COutput>& vCoins, bool fOnlyConfirmed=true, const CCoinControl *coinControl = NULL, const CAmount& nMinimumAmount = 1, const CAmount& nMaximumAmount = MAX_MONEY, const CAmount& nMinimumSumAmount = MAX_MONEY, const uint64_t& nMaximumCount = 0, const int& nMinDepth = 0, const int& nMaxDepth = 9999999, const int nFilterAssetType = -1) const;

    /**
     * Get wallet balance filtered by asset type.
     * @param nAssetType  0x00 = SOQ, 0x01 = USDSOQ
     */
    CAmount GetBalanceByAsset(uint8_t nAssetType) const;
    CAmount GetUnconfirmedBalanceByAsset(uint8_t nAssetType) const;
    ...
    /**
     * Soqucoin: Get a fee targeting a specific transaction speed.
     */
    CAmount GetSoqucoinPriorityFee(const CMutableTransaction& tx, unsigned int nTxBytes, FeeRatePreset nSpeed);
    /**
     * Soqucoin: Get a fee targeting a specific transaction speed.
     */
    static CAmount GetSoqucoinPriorityFee(const CMutableTransaction& tx, unsigned int nTxBytes, FeeRatePreset nSpeed, CAmount targetFee);
    /**
    ...
    /**
     * Return the minimum required fee taking into account the
     * floating relay fee and user set minimum transaction fee, but not the Soqucoin dust fee.
     */
