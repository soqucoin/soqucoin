 ...
 *
 * Description: Generates public and private key deterministically
 ...
 *              - uint8_t *sk: pointer to output private key (allocated
 *                             array of CRYPTO_SECRETKEYBYTES bytes)
 *              - const uint8_t *seed: pointer to input seed (SEEDBYTES bytes)
 *
 * Returns 0 (success)
 **************************************************/
int crypto_sign_seed_keypair(uint8_t* pk, uint8_t* sk, const uint8_t* seed)
{
    ...
    /* Use provided seed instead of random generation */
...
badsig:
    ...
    /* SECURITY NOTE (Halborn FIND-003): Bound zeroing to actual message buffer size.
     * m is allocated for (smlen - CRYPTO_BYTES) bytes, not smlen bytes.
     * The old loop wrote CRYPTO_BYTES (2420) bytes past the end of m. */
    {
        size_t msg_bytes = (smlen >= CRYPTO_BYTES) ? smlen - CRYPTO_BYTES : 0;
