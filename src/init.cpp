...
void Shutdown()
{
    ...
    StopHTTPServer();
#ifdef ENABLE_WALLET
    // Soqucoin 1.14 TODO: ShutdownRPCMining();
    if (pwalletMain)
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/soqucoin/soqucoin>";
    const std::string URL_WEBSITE = "<https://soqucoin.com>";
...
{
    // Dilithium-only: No ECDSA sanity checks needed
...
bool AppInitSanityChecks()
{
    ...
    // Initialize randomness (Dilithium-only, no ECDSA)
    RandomInit();
    globalVerifyHandle.reset(new ECCVerifyHandle()); // Keep for now, will remove in full cleanup
...
bool AppInitMain(boost::thread_group& threadGroup, CScheduler& scheduler)
{
    ...
    // Soqucoin: Do we need to do any RPC mining init here?
