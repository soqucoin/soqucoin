...
Below are some notes on how to build Soqucoin Core for Windows.
...
To build Soqucoin from source code, you'll need the source code. Either check it out via `git` or download
a zip file. (Look at the green "<> Code" button on [the Soqucoin GitHub repository](https://github.com/soqucoin/soqucoin/)).
...
Note that for WSL v1 the Soqucoin Core source path MUST be somewhere in the default mount file system, for
example /usr/src/soqucoin, AND not under, for example, /mnt/d/soqucoin.
...
```
Ensure that this toolchain can build for `posix` (else you may have compilation errors for Soqucoin's dependencies):
```bash
...
way. This will install to `c:\workspace\soqucoin`, for example:
```bash
make install DESTDIR=/mnt/c/workspace/soqucoin
```
