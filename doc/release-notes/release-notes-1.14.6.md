Soqucoin Core version 1.14.6 is now available from:

  <https://github.com/soqucoin/soqucoin/releases/tag/v1.14.6/>

This is a new minor version release, including important security updates and
changes to network efficiency. All Soqucoin Core users - miners, services,
...
Soqucoin network from 1 DOGE to 0.01 DOGE. The full recommendation can be found
...
```
soqucoin-cli addnode <address> remove
```
...
```
soqucoin-cli liststucktransactions
```
...
```
soqucoin-cli setmaxconnections <desired_amount>
```
...
```
soqucoin-cli rescan <start_block>
```
...
efforts to make Soqucoin more accessible to people that do not speak English.
...
* Improved and updated the [bip list](bips.md) documentation that explains the
  protocol features implemented by Soqucoin Core.
