...
if 'ENABLE_WALLET' not in vars():
    ENABLE_WALLET=0
if 'ENABLE_SOQUCOIND' not in vars():
    ENABLE_SOQUCOIND=0
...
#Set env vars
if "SOQUCOIND" not in os.environ:
    os.environ["SOQUCOIND"] = BUILDDIR + '/src/soqucoind' + EXEEXT
...
if not (ENABLE_WALLET == 1 and ENABLE_UTILS == 1 and ENABLE_SOQUCOIND == 1):
    print("No rpc tests to run. Wallet, utils, and soqucoind must all be enabled")
    sys.exit(0)
...
class RPCTestHandler:
    ...
    def __init__(self, num_tests_parallel, test_list=None, flags=None):
        ...
        self.flags = flags
        self.num_running = 0
        # In case there is a graveyard of zombie soqucoinds, we can apply a
...
class RPCCoverage(object):
    ...
    commands per `soqucoin-cli help` (`rpc_interface.txt`).
