...
func NewRPCClient(name, rpcURL, rpcUser, rpcPassword, timeout string) *RPCClient {
	log.Printf("RPC Client Init: URL=%s User=%s Pass=%s", rpcURL, rpcUser, rpcPassword)
	urlParts := strings.Split(rpcURL, "://")
	rpcClient := &RPCClient{
		Name:    name,
		NodeUrl: urlParts[0] + "://" + urlParts[1],
		User:    rpcUser,
		Pass:    rpcPassword,
	}
	...
	rpcClient.client = &http.Client{
		Timeout: timeOutIntv, Transport: &http.Transport{DisableKeepAlives: true},
	}
...
func (r *RPCClient) ValidateAddress(address string) (ValidateAddressResponse, error) {
	...
	if status != 200 {
		return response, fmt.Errorf("RPC: `validateaddress` failed HTTP 200")
	}
...
func (r *RPCClient) GetBlockChainInfo() (BlockChainInfoResponse, error) {
	...
	resp, status, err := r.doRequest("getmininginfo", nil)
	if err != nil {
		return response, err
	}
	if status != 200 {
		return response, fmt.Errorf("RPC: getmininginfo failed. Status: %d, RPC Error: %v", status, resp.Error.Message)
	}
...
func handleHttpError(response rpcResponse, status int) error {
	return fmt.Errorf("HTTP " + strconv.Itoa(status) + ": " + response.Error.Message)
}
