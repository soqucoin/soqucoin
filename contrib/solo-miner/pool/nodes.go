...
// nodeRPCMutex serializes all RPC calls to the node to prevent race conditions
// in getblocktemplate static cache.
var nodeRPCMutex sync.Mutex

type BlockChainNodesMap map[string]blockChainNode
...
type hashblockCounterMap map[string]uint32
...
func (pool *PoolServer) loadBlockchainNodes() {
	...
	for _, blockChainName := range pool.config.BlockChainOrder {
		...
		// FINDING-01 FIX: Retry node RPC during startup with exponential backoff.
		// The node may still be loading the wallet (7000+ txns) or verifying blocks.
		var chainInfo rpc.BlockChainInfoResponse
		var err error
		for attempt := 1; attempt <= 30; attempt++ {
			...
			log.Printf("Node %s not ready (attempt %d/30): %v — retrying in %v",
				blockChainName, attempt, err, backoff)
			time.Sleep(backoff)
		}
		logFatalOnError(err)
...
func (p *PoolServer) submitBlockToChain(block bitcoin.BitcoinBlock) error {
	...
	submit := []any{any(submission)}
	success, err := p.GetPrimaryNode().RPC.SubmitBlock(submit)

	if !success || err != nil {
		nodeName := p.GetPrimaryNode().ChainName
		m := fmt.Sprintf("⚠️  %v primary node rejection: %v", nodeName, err.Error())
		return errors.New(m)
	}
