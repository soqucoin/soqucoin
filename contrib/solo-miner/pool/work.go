...
func (p *PoolServer) recieveWorkFromClient(share bitcoin.Work, client *stratumClient) error {
	...
	// SOQ-INFRA: Rate limit block submissions. On testnet3 (min-diff), the L7
	// finds blocks faster than the node can connect them and rebuild the UTXO set.
	// On mainnet, difficulty adjustment ensures ~60s between blocks naturally.
